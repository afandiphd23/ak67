  ...
  const filtered = query.trim()
    ? allSections
        .filter((s) => {
          ...
          const bm = bmSection(s.id)
          return (
            s.id.toLowerCase().includes(q) ||
            s.number.toLowerCase() === q ||
            s.heading.toLowerCase().includes(q) ||
            (bm && bm.heading.toLowerCase().includes(q)) ||
      ...
      <div className="spotlight-modal" onClick={(e) => e.stopPropagation()}>
        <div className="spotlight-head">
          ...
          <input
            ...
            placeholder={
              lang === 'bm'
                ? 'Cari seksyen, nombor, perkataan (cth: 140, duti, gudang)...'
                : 'Search section number, title, or keywords (e.g. 140, duty, warehouse)...'
            }
          />
        ...
        <div className="spotlight-results">
          {filtered.length === 0 ? (
            <div className="spotlight-empty">
              {lang === 'bm'
                ? 'Tiada seksyen dijumpai.'
                : 'No provisions matching your search.'}
            </div>
          ) : (
            filtered.map((sec, idx) => {
              const bm = bmSection(sec.id)
              const heading = lang === 'bm' && bm?.heading ? bm.heading : sec.heading
              return (
                <div
                  key={sec.id}
