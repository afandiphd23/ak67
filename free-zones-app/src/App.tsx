...
import act, { allSections, findSection, localized, partTitleOf, searchSections } from './data'
...
function getLandingUrl(): string {
  ...
  if (isDev && ['5174', '5175', '5176'].includes(window.location.port)) {
...
function getActUrl(): string {
  ...
  const isDev =
    ...
    window.location.hostname === '127.0.0.1' ||
    window.location.hostname === '[::1]'
  if (isDev && window.location.port === '5176') {
...
function extractSectionPlainText(sec: { content: FlatSection['content'] }): string {
  const parts: string[] = []
...
function Shell() {
  ...
  const [recentIds, setRecentIds] = useState<string[]>(() =>
    JSON.parse(localStorage.getItem('free-zones-recent') ?? '[]'),
  )
  ...
  // Keyboard shortcuts: Ctrl+K spotlight, / focuses search, Esc clears, ← → page between sections.
  useEffect(() => {
...
function SectionView({
  ...
  const copyCitation = useCallback(() => {
    const citation =
      lang === 'bm'
        ? `Akta Zon Bebas 1990 (Akta 438), s. ${section.number} — ${loc.heading}`
        : `Free Zones Act 1990 (Act 438), s. ${section.number} — ${loc.heading}`
  ...
  const copyMarkdown = useCallback(() => {
    const title = lang === 'bm' ? 'Akta Zon Bebas 1990 (Akta 438)' : 'Free Zones Act 1990 (Act 438)'
    ...
    <article className="section-view">
      ...
      <div className="section-head">
        <div>
          ...
          <h3 className="sec-title">{lang === 'bm' ? act.meta.bmTitle : act.meta.title}</h3>
        </div>
        <div className="section-tools">
          <button
            className="print-btn"
...
function ScheduleBlock({ schedule: s, lang }: { schedule: (typeof act.schedules)[number]; lang: Lang }) {
  const entry = lang === 'bm' ? s.bm : null
  const label = entry?.label ?? (lang === 'bm' ? s.bmLabel : s.label)
  const ref = entry?.ref ?? s.ref
  const blocks = entry?.blocks ?? s.blocks
  const headingLabel =
    s.id.startsWith('sched-')
      ? `${lang === 'bm' ? 'JADUAL' : 'SCHEDULE'} — ${label.toUpperCase()}`
      : label.toUpperCase()
  return (
    <section className="schedule-block">
      <h3 className="schedule-heading">
        {headingLabel}
        {ref && <span className="schedule-ref"> {ref}</span>}
      </h3>
      {s.id.startsWith('sched-') && lang === 'bm' && !s.bm && (
        <p className="translation-note">{UI.bm.translatedNote}</p>
      )}
      {blocks.map((b, i) => (
  ...
  return (
    <div className="welcome">
      ...
      <div className="stats">
        ...
        <div>
          <strong>{act.schedules.filter((s) => s.id.startsWith('sched-')).length}</strong> {t.scheduleStat}
        </div>
      </div>
      <Disclaimer />
